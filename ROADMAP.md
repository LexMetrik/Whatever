...
- [ ] **Gesetz-Leser V3 — Hülle neu, Kern unangetastet** *(`W2·5m-LESER-V3`, Auftrag David 16.8.2026)*
  ...
  **Detail:** [FAHRPLAN-LESER-V3.md](fahrplaene/FAHRPLAN-LESER-V3.md) (Kurzfassung zuoberst; Kap. 7 Etappen H1–H5/S1–S4, Kap. 9 Fragen F1–F6).
  ...
  - [ ] **Gliederung: Standort sichtbar, Mitlaufen beim Lesen, Auf/Zu-Handling** *(David 15.9.2026: «gliederung ist vom ui handling noch schwerfällig … man erkennt nicht so gut wo man sich befindet»)* — aktiver Abschnitt/Artikel deutlich markiert und in der Seitenleiste im Blick gehalten; Auf-/Zuklappen mit weniger Klicks (Nachbarn: «ganz auf/zu» Fahrplan LESER-V3 #16, Klickpfad 161 ms QS-PERF-Rest, Knoten-Merge-Bug FAHRPLAN-OFFENE-BEFUNDE §Gliederungsbaum). Mehrwert-Satz: Fedlex hat keine mitlaufende Gliederung. **Marke + Klickziel vorgezogen PR #894 (15.9.2026)**; offen: Mitlaufen bei schnellem Scroll (Marke erst ~300 ms nach Scroll-Stopp, `AUTO_AUF_RUHE_MS`), Auf/Zu-Handling, Standort-Fläche trägt allein nicht (`brass-100` 1,03–1,13:1 gegen Leiste/Blatt — Token-Entscheid nötig).
...
- [~] **Phase 1 · Bund fertig machen — Sollbild und Struktur-Schluss** *(`W2·27-BUND-FERTIG`, Entscheid David 14.9.2026)*
  ...
  **Detail:** [FAHRPLAN-BUND-FERTIG.md](fahrplaene/FAHRPLAN-BUND-FERTIG.md) §3.
  ...
  - [ ] **Staatsverträge ohne Randtitel und Historie — Befund korrigiert 15.9.2026, Rest = «gilt seit»-Projektion** — es sind 17 Staatsverträge (CISG, LugÜ, HKÜ, HZÜ, HBewÜ, UNO-Pakte I/II, CEDAW, Antifolter, HAÜ, HUÜ, RBÜ, Apostille, CMR, HEÜ, HKsÜ96, KRK) + VBB (SR 281.31) und VG (SR 170.32). **Randtitel: amtlich nicht vorhanden** (Fedlex-`<h6>` trägt nur «Art. N»; `artikelSachtitel` funktioniert, Beleg EMRK 59/60) ⇒ nichts zu extrahieren (§7). **Historie:** LugÜ-Änderungs-Fussnoten hängen an Anhang-Headings → `W2·5l-NORMTEXT-B2` M13; CMR 4 Ereignisse ohne Datum in der Quelle; KRK/VBB/VG haben Historie. **Offen (Leseoberfläche, kein Risikopfad):** Artikel ohne Historie-Ereignis zeigen «Erlass in Kraft seit <Datum> · keine artikelgenaue Änderungshistorie erfasst» als Projektion aus `inkrafttreten.json` (`jolux:dateEntryInForce`, deckungsgleich mit dem Fedlex-Vorspann); Mehrwert: Fedlex zeigt das Inkrafttreten nur im Vorspann. Spec liegt bereit (Übergabe 15.9.2026 (3), `w227-staatsvertraege-spec.md`); Fahrplan BUND-FERTIG §2 Z.3 mit dieser Korrektur nachgezogen (15.9.2026).
  - [ ] **Aufhebungs-Signal: Anhänge/`<section>` und «Gegenstandslos»** *(Prüfer #892, 15.9.2026)* — `scripts/normtext/aufhebung-signal.ts` erkennt nur «Aufgehoben durch»; StGB Art. 67f «Gegenstandslos gemäss …, mit Wirkung seit 1.1.2018» bleibt Klasse «leer-ungeklärt». Signal erweitern, Sonder-Sektionen mitnehmen. *Risikopfad ⇒ Gegenprüfung.*
  - [ ] **Absatz-genaues Aufhebungs-Signal zur Bauzeit** *(Bauer #892, 15.9.2026)* — heute pauschal je Artikel; 872 Bund-Absätze (Historie-Shard) und ~869 Kanton-Absätze (`abrogation_ellip` je `<p>`) könnten absatzgenau «aufgehoben» tragen (Snapshot-Schema-Feld). Heimat hier oder `W2·5l-NORMTEXT-B2`. *Risikopfad ⇒ Gegenprüfung.*
  - [ ] **`verifikationslink.ts` unterdrückt den Deep-Link bei `__N`-Token noch ganz** *(Nebenfund Fix #890)* — seit `amtlicherAnker()` gibt es einen korrekten Anker (`#ta126z`); Deep-Link freigeben, wenn `quelleUrl` einen amtlichen Fragment-Anker trägt. Leseoberfläche, klein.
  - [ ] **`artikelLabel`/`quelleUrl` ohne Drift-Tor** *(§8, Fahrplan §1.1, PR #890)* — beide sind golden-neutral, eine stille Änderung je Erlass fiele heute nicht auf; Kandidat: Riegel in `check:normtext` (Zählung je Erlass, Änderung nur deklariert).
  - [ ] **Frische-Tor-Lücke Umbenennung** *(Prüfer #888)* — reine Umbenennung einer Bund-Snapshot-Datei bewegt weder `artikel.sha` noch die Dateizahl, `confidence.json` bliebe still alt. Schmale Klasse; Kandidat: Dateinamen-Liste in den Manifest-Hash.
...
- [ ] **Entstehung am Artikel — Bulletin-Voten Bundesrat/Kommission: Metadaten + Deep-Link** *(`W2·6d-BULLETIN-VOTEN`, 15.9.2026; Phase 3)*
  ...
  Voten nach Funktion, Rat, Datum, Lesung mit Deep-Link ins Amtliche Bulletin; Artikel-Zuordnung
