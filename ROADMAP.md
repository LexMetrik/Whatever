...
- [ ] **Gesetz-Leser V3 — Hülle neu, Kern unangetastet** *(`W2·5m-LESER-V3`, Auftrag David 16.8.2026)*
  ...
  **Detail:** [FAHRPLAN-LESER-V3.md](fahrplaene/FAHRPLAN-LESER-V3.md) (Kurzfassung zuoberst; Kap. 7 Etappen H1–H5/S1–S4, Kap. 9 Fragen F1–F6).
  ...
  - [ ] **Gliederung: Standort sichtbar, Mitlaufen beim Lesen, Auf/Zu-Handling** *(David 15.9.2026: «gliederung ist vom ui handling noch schwerfällig … man erkennt nicht so gut wo man sich befindet»)* — aktiver Abschnitt/Artikel deutlich markiert und in der Seitenleiste im Blick gehalten; Auf-/Zuklappen mit weniger Klicks (Nachbarn: «ganz auf/zu» Fahrplan LESER-V3 #16, Klickpfad 161 ms QS-PERF-Rest, Knoten-Merge-Bug FAHRPLAN-OFFENE-BEFUNDE §Gliederungsbaum). Mehrwert-Satz: Fedlex hat keine mitlaufende Gliederung. **Marke + Klickziel vorgezogen PR #894**; offen: Mitlaufen (Marke erst ~300 ms nach Scroll-Stopp), Auf/Zu, Standort-Fläche (`brass-100` 1,03–1,13:1 — Token-Entscheid).
...
- [~] **Phase 1 · Bund fertig machen — Sollbild und Struktur-Schluss** *(`W2·27-BUND-FERTIG`, Entscheid David 14.9.2026)*
  ...
  **Detail:** [FAHRPLAN-BUND-FERTIG.md](fahrplaene/FAHRPLAN-BUND-FERTIG.md) §3.
  ...
  - [~] **Randtitel-Doppelmodell auflösen** *(Entscheid David 14.9.2026, Fahrplan §4 c: `struktur.marginalie` wird die eine Quelle)* — **Phase-1-Schnitt ✅ 15.9.2026, PR #889 (`d20e3f5d3`, Gegenprüfung bestanden 5a93ac060)**, Wortlaut: ROADMAP-CHRONIK.md, Umschichtung 15.9.2026 (4). **Rest Phase 2 (Fahrplan §1.2):** Kanton `titel` → `marginalie`, Suchindex ohne Fallback, NormChip, zweiter Kanton-Parser `struktur-lexwork.ts:317`; der `titel`-Zweig ist heute toter Code (Prüfer B3) ⇒ evtl. ersatzlos rückbaubar.
  - [x] **§8-Anzeige der ungeklärten Leerstellen** *(Auflage Gegenprüfung #859)* — ✅ 15.9.2026, PR #892 (`674cc42ae`, Gegenprüfung bestanden dbe25de86): «kein Text im Snapshot» statt «aufgehoben» ohne Vermerk. Wortlaut: ROADMAP-CHRONIK.md, Umschichtung 15.9.2026 (4).
  - [x] **§17 · `confidence.json` ohne Frische-Tor** *(Wurzel-Befund Prüfer #848)* — ✅ 15.9.2026, PR #888 (`3202047ae`, Gegenprüfung bestanden af1d214a3): Tor `check:confidence-frische` koppelt `korpus.sha` ans Manifest. Wortlaut: ROADMAP-CHRONIK.md, Umschichtung 15.9.2026 (4).
  ...
  - [ ] **Staatsverträge ohne Randtitel und Historie — Befund korrigiert 15.9.2026, Rest = «gilt seit»-Projektion** — 17 Staatsverträge (CISG, LugÜ, CMR …) + VBB/VG. Randtitel amtlich nicht vorhanden (Fedlex-Heading nur «Art. N», §7); Historie: LugÜ = Anhang-Fussnoten → `W2·5l` M13, CMR ohne Datum, KRK/VBB/VG haben Historie. **Offen (Leseoberfläche):** Artikel ohne Historie-Ereignis zeigen «Erlass in Kraft seit …» als Projektion aus `inkrafttreten.json`. Spec: Fahrplan BUND-FERTIG §2 Z.3, `w227-staatsvertraege-spec.md` (Übergabe).
...
- [ ] **Davids Alltags-Fehlerfunde** *(`W2·18-FEHLERBUCH`, stehender Sammel-Schritt, Entscheid David 8.8.2026)*
  ...
  **Detail:** [FAHRPLAN-OFFENE-BEFUNDE.md](fahrplaene/FAHRPLAN-OFFENE-BEFUNDE.md) §4 — dort die
  ...
  - [x] **Gliederungs-Pfeil klappt teils erst beim zweiten Klick** *(David 15.9.2026)* — ✅ 15.9.2026, PR #894 (`fb0ab0249`): Ast bleibt nach Artikel-Sprung offen (`sprungAst.ts`), Klickziel 24 px, Marke Strich + Fläche; Rest unter `W2·5m-LESER-V3`. Wortlaut: ROADMAP-CHRONIK.md, Umschichtung 15.9.2026 (4).
