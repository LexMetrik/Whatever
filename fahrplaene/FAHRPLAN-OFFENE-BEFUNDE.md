...
- *6 erledigte Einträge dieses Abschnitts wörtlich in [`archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md`](../archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md), Abschnitt «§1 — `QS-KORPUS` · Korpus-Pflege: fehlende und fehlerhafte amtliche Substanz» (verschoben 24.9.2026).*
...
- *2 erledigte Einträge dieses Abschnitts wörtlich in [`archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md`](../archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md), Abschnitt «Restposten aus ROADMAP.md *(verlagert 20.9.2026)*» (verschoben 24.9.2026).*
...
Herkunft: `ROADMAP.md`, Dach-Schritt `QS-MONITOR-ROT`. Anlass: der 120-KB-Deckel des Plans —
der erste Umzug am 20.9.2026 landete 483 B unter dem Deckel, ein paralleler Merge riss ihn
wieder; dieser Nachzug schafft Reserve. In `ROADMAP.md` bleiben Ziel, Risiko-Klassierung, die
harten Auflagen (Gegenprüfungs-Auflagen PR #623, Merge-Sperre Kanton-Drift, Gegenprüfung
Curia-Monatslauf #939), die Fristen (Pflegetermin 1.10.2026, Ablauf 1.10.2026), der
David-Entscheid «falsches rectifies-Ziel» und der Zeiger auf diesen §2; die Befund- und
Beleg-Historie steht ab hier. **Wortlaut byte-genau, nichts zusammengefasst, nichts gestrichen.**
Die Auflage des Dach-Schritts gilt unverändert für jede Zeile: Risikopfad ⇒ Gegenprüfung
Pflicht, amtlicher Beleg mit Norm + Link + Stand (§7), Korrektur nie in der Projektion, immer
in der Pipeline-Quelle (§5).
...
**Messung 20.9.2026 — Stichtag 22.9. bestätigt, befristete Ausnahmen eingetragen (Entscheid David 20.9.2026):** 46 von 46 Code-Läufen (30 `pull_request` + 16 `merge_group`, 18.–20.9.2026) hatten mindestens eine ungedeckte Flake — eigene Stichprobe (20 von 46 Läufen, alle mit tatsächlich ausgeführten Playwright-Shards statt Doku-Skip) bestätigt dasselbe Bild: `uinav-j-rechtsprechung.e2e.ts` 20/20, `leser-v3-suche-ohne-gliederung.e2e.ts` und `w224-r11-reiterleiste.e2e.ts` je 19/20, `leser-v3-blatt.e2e.ts` 7/20, dazu vereinzelt `verweis-u.e2e.ts`, `w224-plus-reiter.e2e.ts`, `rechtsprechung-richter.e2e.ts`, `leser-ruecksprung-r5-r7.e2e.ts` (je 1/20) und `rechtsprechung.e2e.ts` (in der eigenen Stichprobe nicht getroffen, aber am 19.9. belegt, s. Zeile 288 oben). `leser-r1-r2.e2e.ts` ist seit PR #929 (19.9.2026, 17:35 Uhr) gefixt — die zwei Flake-Sichtungen dieser Spec in der Stichprobe datieren beide VOR dem Merge (17:13 und 17:24 Uhr) und sind damit kein Gegenbeleg. Der Wächter wird am 22.9.2026 wie geplant hart (`e2e/flake-modus.json` unverändert); für die neun oben genannten Specs stehen befristete Ausnahmen bis **20.10.2026** in `e2e/flake-ausnahmen.json` (`w224-reiterverhalten.e2e.ts` bleibt bei ihrem bestehenden Verfall 8.10.2026, unverändert). **Wurzeln je Spec messen, nicht Ausnahmen verlängern** — bei Ablauf ohne Wurzel-Fix wird der Wächter rot, das ist gewollt (§6.7).
...
- *2 erledigte Einträge dieses Abschnitts wörtlich in [`archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md`](../archiv/FAHRPLAN-OFFENE-BEFUNDE-erledigt.md), Abschnitt «§5 — `QS-CODE-PROP` · Eigenschafts-Tests (property-based) für die Rechen-Engines» (verschoben 24.9.2026).*
