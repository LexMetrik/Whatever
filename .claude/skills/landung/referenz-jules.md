...
8. Danach normale Landung (Schritte 0–3) wie bei eigener Arbeit. Seit 4.9.2026 fährt CI dieselben Regeln automatisch (Step «Fremd-PR-Tor» im Job «Tore», Branches mit 19-stelliger Task-ID oder Präfix `jules-`/`jules/` — nie «jules» irgendwo im Namen, Beleg PR #656): keine Datei ausserhalb `src/**` + Kommentar-Bilanz (Regel 3 + 3b, `scripts/analyse/kommentar-bilanz.ts`); Assertions seit 23.9.2026 nur `check:fachaenderung` (Risiko-Engines) — der lokale Lauf bleibt Pflicht (§14.7, nie die CI-Ausgabe eines Fremden als eigene Prüfung zählen).
